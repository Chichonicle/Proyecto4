...
const register = async (req: Request, res: Response) => {
  try {
    ...
    await User.update({ id: updateworker!.id }, { role: "admin" });
...
const workerAppointments = async (req: Request, res: Response) => {
  try {
    const workerId = req.token.id;
    const pageSize: any = parseInt(req.query.skip as string) || 9999;
    ...
    const filteredAppointments = workerAppointments.map((appointment) => ({
